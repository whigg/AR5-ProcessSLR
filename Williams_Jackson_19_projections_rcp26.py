...
## Empty array to store model mean information
...
minor_yticks = np.arange(0, 1.5, 0.125)
major_yticks = np.arange(0, 1.5, 0.25)
...
for i in range(m):
    ...
    C, F, T   = fair_scm(emissions = rcp3pd.Emissions.emissions)
    ...
    # shift mean, so relative to 1986-2005 mean
    ...
    # Glaciers - same as in de Vries, 2014.
    ...
    ## --- uncomment all this if you want subplots with temperature spaghetti pathways and individual process contributions --- ##
...
#plt.plot(time_2000, np.divide(model_mean, 28), 'k--')

ax.plot(time_2000, np.divide(model_mean+model_mean[5],28), color = 'brown', linestyle = '--', linewidth = 1)
ax.errorbar(2100, np.divide(model_mean+model_mean[5],28)[99], yerr=[[np.divide(model_mean-model_mean[5],28)[99]-0.79],[1.4 - np.divide(model_mean-model_mean[5],28)[99]]], color = 'orange')
ax.scatter(2100, np.divide(model_mean+model_mean[5],28)[99], marker = 'x', color = 'orange', zorder = 1, label = 'Multi-model mean, this work')

ax.scatter(2103, 0.52, marker = 'x',color = 'blue',label = 'Grinsted et al. (2010) multi-model mean')
ax.errorbar(2103, 0.52, yerr=[[0.52-0.25],[0.89-0.52]], color = 'blue')


ax.scatter(2105, 0.38, marker = 'x',color = 'red',label = 'Kopp et al. (2016) multi-model mean')
ax.errorbar(2105, 0.38, yerr=[[0.38-0.28],[0.51-0.38]], color = 'red')

ax.scatter(2107, 0.43, marker = 'x',color = 'green',label = 'AR5 multi-model mean')
ax.errorbar(2107, 0.43, yerr=[[0.43-0.28],[0.61-0.43]], color = 'green')
